...
export function ReportDetailView({ report, onBack }: ReportDetailViewProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.text(`Patient Name: ${report.patientName}`, margin, yPosition);
    yPosition += 5;
    pdf.text(`MRN: ${report.patientId}`, margin, yPosition);
    yPosition += 5;
    pdf.text(`Study Date: ${report.studyDate}`, margin, yPosition);
    yPosition += 5;
    pdf.text(`Modality: ${report.modality}`, margin, yPosition);
    yPosition += 5;
    pdf.text(`Body Part: ${report.bodyPart}`, margin, yPosition);
    yPosition += 5;
    pdf.text(`Radiologist: ${report.radiologist}`, margin, yPosition);
    yPosition += 10;
    ...
    report.images.forEach((img, idx) => {
      pdf.text(`${idx + 1}. ${img.label}`, margin + 5, yPosition);
      yPosition += 5;
    });
  ...
  const getImageSrc = (imageUrl: string) => {
    if (imageUrl === 'ct-brain') return ctImage;
    if (imageUrl === 'mri-brain') return mriImage;
    return imageUrl;
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button
                ...
                size="sm"
                variant="ghost"
                className="text-white/80 hover:text-white hover:bg-[#3E3E42]"
              >
              ...
              <Button
                ...
                className={`${
                  isChatOpen 
                    ...
                    : 'bg-transparent border-[#3E3E42] text-white/80 hover:bg-[#3E3E42] hover:text-white'
                }`}
              >
          ...
          <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            <div className="bg-[#2B2B2B] border border-[#3E3E42] rounded-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-sm text-white/60">Patient Name:</span>
                  <p className="text-white/90">{report.patientName}</p>
                </div>
                <div>
                  <span className="text-sm text-white/60">MRN:</span>
                  <p className="text-white/90">{report.patientId}</p>
                </div>
                <div>
                  <span className="text-sm text-white/60">Study Date:</span>
                  <p className="text-white/90">{report.studyDate}</p>
                </div>
                <div>
                  <span className="text-sm text-white/60">Modality:</span>
                  <p className="text-white/90">{report.modality}</p>
                </div>
                <div>
                  <span className="text-sm text-white/60">Body Part:</span>
                  <p className="text-white/90">{report.bodyPart}</p>
                </div>
                <div>
                  <span className="text-sm text-white/60">Radiologist:</span>
                  <p className="text-white/90">{report.radiologist}</p>
                </div>
                <div>
                ...
                <div>
                  <span className="text-sm text-white/60">Report Type:</span>
                  <p className="text-white/90 capitalize">{report.reportType}</p>
                </div>
              </div>
            </div>
      ...
      {isChatOpen && (
        <div className="w-96 border-l border-[#3E3E42] flex flex-col bg-[#1E1E1E] h-full">
          ...
          <div className="p-4 border-b border-[#3E3E42] flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
              <MessageSquare className="h-5 w-5 text-[#00A9E0]" />
              <h3 className="text-white/90">AI Assistant</h3>
            </div>
            <Button
              onClick={() => setIsChatOpen(false)}
              size="sm"
              variant="ghost"
              className="text-white/60 hover:text-white hover:bg-[#3E3E42] h-8 w-8 p-0"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
                  ...
                  <div
                    className={`max-w-[80%] rounded-lg p-3 ${
                      message.role === 'user'
                        ...
                        : 'bg-[#2B2B2B] text-white/90 border border-[#3E3E42]'
                    }`}
                  >
          ...
          <div className="p-4 border-t border-[#3E3E42] flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#2B2B2B] border border-[#3E3E42] rounded px-3 py-2 text-sm text-white/90 placeholder:text-white/40 focus:outline-none focus:border-[#00A9E0]"
              />
              <Button
            ...
            <p className="text-xs text-white/40 mt-2">
              AI Assistant can help explain findings and answer questions
            </p>
          </div>
        </div>
      )}
